...
ScavTrap::ScavTrap(){
	...
	_ranged_ad = 15;
	_armor_dr = 3;
	std::cout << "Default Scav is here! Be ready for the challenge" << std::endl;
}
...
ScavTrap::ScavTrap(const std::string& name) : _name(name){
	...
	_ranged_ad = 15;
	_armor_dr = 3;
	std::cout << "Scav " << _name <<" is here! Be ready for the challenge" << std::endl;
}
...
ScavTrap::~ScavTrap(){
	std::cout << "Scav " << _name << " has gone away! This time you won!" << std::endl;
}

ScavTrap &	ScavTrap::operator=( ScavTrap const & rhs ){
	std::cout << "Assignation operator of Scav called" << std::endl;
...
void	ScavTrap::takeDamage(unsigned int amount){
	...
	if (_hit_points <= 0){
		_hit_points = 0;
		std::cout << "Scav " << _name << " now is dead!!! Now it's not funny:(" << std::endl;
	}
	else{
		std::cout << "Scav " << _name << " is damaged on "
...
void	ScavTrap::beRepaired(unsigned int amount){
	...
	std::cout << "Scav " << _name << " has repaired to HP "
...
void	ScavTrap::challengeNewcomer(std::string const & target){
	...
	"draw a picture", "take picture with Scav"};
	_energy_points -= 15;
	if (_energy_points <= 0){
		_energy_points = 0;
		std::cout << "Scav " << _name << " don't have enough energy to challenge newcomer! Oh no!" << std::endl;
	}
	else{
		std::cout << "Scav " << _name << " has a challenge to " << target
